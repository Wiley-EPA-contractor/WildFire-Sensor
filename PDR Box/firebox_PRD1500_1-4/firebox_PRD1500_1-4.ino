...
void pdr1500()
{
 ...
 if(first_run){ // let bad data clear out
 delay (3000);
 first_run = false;
 }
 delay(100);
  ...
  while(PDRSerial.available() > 0) 
 {
  ...
  if (data_pointer > 8){
  if (PDRChar == '\r' || x == 13 ){ // for some reason, not all return characters are being recognized.
    if (!data_is_valid){
    break;}  
    else{
  ...
  else if (((x == 32 || x == 46 ) || (x>=48 && x<= 57))){// ||(x>=65 && x<=90) || (x>=97 && x<=122) )) {  // I honestly hate this line of code but it basically 
  data_is_valid = true;
  valid_data_length++;
  //checks for "normal" characters when the Character is loaded in
  //Normal Chars: 32 == space, 46 == period, 62 == >, 48<=x<=57 == 0-9, 65<=x<=90 == A-Z, 97<=x<=122 == a-z
